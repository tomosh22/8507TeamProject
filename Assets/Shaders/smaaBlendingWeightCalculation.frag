...
void main() {
  ...
  if (e.g > 0.0) { // Edge at north
    ...
    weights.rg = SMAACalculateDiagWeights(edgesTex, areaTex, IN.texCoord, e, subsampleIndices);
    ...
    vec3 coords;
    coords.x = SMAASearchXLeft(edgesTex, searchTex, IN.offset[0].xy, IN.offset[2].x);
    coords.y = IN.offset[1].y; // vOffset[1].y = vTexCoord0.y - 0.25 * SMAA_RT_METRICS.y (@CROSSING_OFFSET)
    d.x = coords.x;
    ...
    d = abs(round(mad(SMAA_RT_METRICS.zz, d, -IN.pixelCoord.xx)));
  ...
  if (e.r > 0.0) { // Edge at west
    ...
    vec3 coords;
    coords.y = SMAASearchYUp(edgesTex, searchTex, IN.offset[1].xy, IN.offset[2].z);
    coords.x = IN.offset[0].x; // vOffset[1].x = vTexCoord0.x - 0.25 * SMAA_RT_METRICS.x;
    d.x = coords.y;
    ...
    coords.z = SMAASearchYDown(edgesTex, searchTex, IN.offset[1].zw, IN.offset[2].w);
    d.y = coords.z;

    // We want the distances to be in pixel units:
    d = abs(round(mad(SMAA_RT_METRICS.ww, d, -IN.pixelCoord.yy)));
