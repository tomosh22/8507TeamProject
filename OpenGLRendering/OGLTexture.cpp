...
TextureBase* OGLTexture::RGBATextureFromData(char* data, int width, int height, int channels) {
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_RGBA8, width, height, 0, sourceType, GL_UNSIGNED_BYTE, data);
...
TextureBase* OGLTexture::RGBATextureFromFilename(const std::string&name, bool flipped) {
	char* texData	= nullptr;
	int width		= 0;
	int height		= 0;
	int channels	= 0;
	int flags		= 0;
	TextureLoader::LoadTexture(name, texData, width, height, channels, flags,flipped);  
